...
| vref bandgap | ⏳ **partial — iq structural floor** | **PTAT/CTAT bandgap** + **real NMOS diff-pair error amp** (opamp blocks, not VCVS). `.op` converges; V(ra1)≈V(qp1). **Re-verified 2026-06-20** (`scripts/verify_phase3_vref.py` defaults + `scripts/diag_vref_iq_breakdown.py`): defaults **vref=1.146 V**, **line_reg=1.30 mV** ✅, **iq=203.8 µA** ❌ (+104% vs `<100 µA` bar); sized seed=42 budget=80 **vref=1.196 V**, **line_reg=1.25 mV** ✅, **iq=138.5 µA** ❌ (+38% vs bar). **Diagnose-before-sizing:** manual `iref_amp_uA` 15→1 µA only drops iq to **166 µA** — still 66% over bar; `ibias_uA`/`mirror_w` sweeps negligible. **Sizer param_ranges omit amp bias** — but manual sweep shows **~166 µA floor**, not closable to 100 µA by bias alone. See `docs/semicon-log.md` entry 3. BJTs still **level-1 placeholders**. Default vref margin (34 mV below 1.18 V) secondary to iq. |
