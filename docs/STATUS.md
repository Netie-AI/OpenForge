...
| opamp CMRR @100 Hz (bundled) | **152.0 dB** | **151.4 dB** (seed=42) | RS321 typ **80 dB** | **Option B honest partial** — bench-only, not in `DEV_MODE_SPECS`. Normalization corrected; noise-floor refuted (acm_vm 47–272 µV, dv-verifier confirmed). Lb causal bundled + BSIM; bundled-only artifact refuted. **Mismatch ceiling:** zero intentional device mismatch in deck — datasheet CMRR is mismatch-limited; **55 dB gap not closeable by fixture topology alone** (Monte Carlo Phase 4+, `PARKING_LOT.md`). RL=10k feedback fixture experiment queued (~10–15 dB move expected); then stop diagnostic churn. **No fixture policy lock.** |
