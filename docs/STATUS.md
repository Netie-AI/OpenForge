...
- **0.9 device-aware routing (2026-06-21):** Fixed the `n1` mirror-tie slicing straight through **M4's transistor body** to reach its gate (the flagged `y=156` jog across `x[344,370]`). Two-part fix, both in the browser render path:
  ...
  - **Evidence:** regenerated `logs/schematic_opamp_m4_fix.svg`; opamp `crossing_score=2` (two `n1`/`nout1` hops; **0** active-body slices); invariant check `scripts/diag_schematic_invariants.py` → `ALL_INVARIANTS_OK=True` (opamp+comparator, 0 slices / 0 cross-net collinear overlaps). Tests: `test_schematic_connectivity.py` **42/42**, `test_schematic_no_tangling.py` **7/7** (2 new slice regressions), `test_netlist_schematic.py` **13/13**.
