...
- **0.8+ tangling guard (placement pressure):** Cherry-pick landed (`ef43ef6`). `schematic_geometry.py` + opamp placement-variant scoring (`tail_aligned` vs `isolated`) on production `route_nets()` path. **Passive tap second pass (2026-06-20):** Miller cap (`Cc`) stubs onto existing net wires in `schematic_router.py`; **`crossing_score` 6→3** (target ≤3 met). Tests **5/5** tangling + **14/14** connectivity (**19/19** combined). Root cause confirmed: `nb` net placement span (M8/M5/M7), not routing draw bug. `files/` bundle reference-only.
