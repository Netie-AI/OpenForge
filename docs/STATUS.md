...
| opamp CMRR @100 Hz (bundled) | **152.0 dB** | **151.4 dB** (seed=42) | RS321 typ **80 dB** | Normalization corrected (+20 dB artifact fixed). **152 dB not physically credible** on bundled models — PSRR 20–55 dB vs CMRR 150+ dB is inconsistent. Tail/bias sweep (`diag_opamp_cmrr_breakdown.py`): **Lb** (M8) 0.5→8 µm drops CMRR **159.5→125.5 dB**; rl10k fixture ~**15–25 dB** below open-loop on defaults. **BSIM Lb sweep (2026-06-20):** 168.4→135.7 dB open-loop, 142.8→128.1 dB rl10k — Lb causal on BSIM but **more** inflated at short Lb than bundled; bundled-only artifact hypothesis **refuted**. **No production-fixture policy lock.** Bench-only; datasheet-equivalence `unverified` (`semicon-log.md` entry 6). |
...
**SKY130/BSIM CI status:** Local SKY130/BSIM verified at pushed HEAD `85a8d51` (WSL smoke **5/5**, vref deferred; parent rerun 2026-06-20). GitHub Actions has `sky130-bsim-smoke` job in `.github/workflows/ci.yml`. **Actions green URL not verified:** workflow triggers on `main`/`master` push + `pull_request` only — no open PR on `feat/schematic-orthogonal-router` and no pull-request workflow runs for this branch; open [compare → PR](https://github.com/Netie-AI/OpenForge/compare/main...feat/schematic-orthogonal-router) to close gate.
...
| CI | ⚠️ **workflow wired; Actions proof pending PR** | Commits **`ef43ef6`**/`a8e8097`/`85a8d51` pushed on `feat/schematic-orthogonal-router`. Last known bundled-only run **#19** (`7dc7182`). Local WSL BSIM smoke **5/5** at **`85a8d51`**. **Not verified:** GitHub Actions run with `sky130-bsim-smoke` green (needs PR — compare link in header above). |
...
- **0.8+ tangling guard (placement pressure):** Cherry-pick landed (`ef43ef6`). `schematic_geometry.py` + opamp placement-variant scoring (`tail_aligned` vs `isolated`) on production `route_nets()` path. Tests **5/5** + **14/14**. **`crossing_score=6`** (partial; target ≤3). Root cause confirmed: `nb` net placement span (M8/M5/M7), not routing draw bug. `files/` bundle reference-only. Next: Cc passive tap second pass.
