...
- **0.8+ tangling guard:** **`crossing_score` target met (≤3).** Passive tap second pass (`schematic_router.py`, 2026-06-20): **6→3**; pytest **19/19** (5 tangling + 14 connectivity). Variant `tail_aligned`; residual 3 crossings localized at Miller cap — acceptable, not blocking. `nb` span **300→174** from placement variant. `files/` bundle reference-only.
