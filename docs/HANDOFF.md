...
| **2** | **CMRR causal diagnosis (open)** | Bundled Lb sweep: **159.5→125.5 dB** (0.5→8 µm). **BSIM Lb sweep done** (`--lb-only`): **168.4→135.7 dB** — bundled-only artifact hypothesis **refuted**. **ACM noise-floor probe done:** acm_vm **47–272 µV**, CM stimulus verified at both inputs — **not** simulator floor division. PSRR flat 20 dB across BSIM Lb sweep. **152–168 dB still implausible** vs RS321 typ 80 dB. **No production-fixture policy.** Next: RS321 RL=10k **feedback** fixture / deck equivalence |
